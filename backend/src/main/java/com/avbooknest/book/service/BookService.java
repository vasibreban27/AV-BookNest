...
public class BookService {
    ...
    public List<BookResponse> listMine(String email) {
        return bookRepository.findAllBySellerIdOrderByCreatedAtDesc(currentUser(email).getId()).stream().map(BookResponse::from).toList();
    }
    ...
    public BookResponse create(BookRequest request, String email) {
        ...
        Book book = Book.builder()
                .title(request.title().trim()).author(request.author().trim()).isbn(trimToNull(request.isbn()))
                .description(trimToNull(request.description())).price(request.price()).bookCondition(request.bookCondition())
                .language(request.language().trim()).publisher(trimToNull(request.publisher())).publishedYear(request.publishedYear())
                .coverImageUrl(trimToNull(request.coverImageUrl())).seller(currentUser(email)).category(findCategory(request.categoryId()))
                .status(request.status() == null ? BookStatus.AVAILABLE : request.status()).createdAt(now).updatedAt(now).build();
    ...
    public BookResponse update(Long bookId, BookRequest request, String email) {
        ...
        book.update(request.title().trim(), request.author().trim(), trimToNull(request.isbn()), trimToNull(request.description()),
                request.price(), request.bookCondition(), request.language().trim(), trimToNull(request.publisher()), request.publishedYear(),
                trimToNull(request.coverImageUrl()), findCategory(request.categoryId()), request.status() == null ? book.getStatus() : request.status());
