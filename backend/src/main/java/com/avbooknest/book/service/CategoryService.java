...
public class CategoryService {
    ...
    public CategoryResponse create(CreateCategoryRequest request, String email) {
        ...
        if (categoryRepository.existsByNameIgnoreCase(request.name().trim()) || categoryRepository.findBySlug(slug).isPresent()) {
            throw new ConflictException("A category with this name or slug already exists");
        }
        Instant now = Instant.now();
        Category category = Category.builder().name(request.name().trim()).slug(slug)
